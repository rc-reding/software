...
def plot_tree(tree, align_names=False, name_offset=None, name_colors='black', max_dist=None, ms=0, font_size=9, axe=None, **kwargs):
    """
    ...
    for n in node_list:
        ...
        if n.is_leaf():
            y = node_pos[n]
            leaf_color = name_colors[n.name] if type(name_colors) == dict else name_colors
            leaf_weight = 'normal' if leaf_color == 'black' else 'bold'
            if align_names:
                axe.text(max_x + name_offset, y, n.name, color=leaf_color,
                         va='center', size=font_size, fontweight=leaf_weight)
                ali_lines.append(((x, y), (max_x + name_offset, y)))
            else:
                axe.text(x + name_offset, y, n.name, color=leaf_color,
                         va='center', size=font_size, fontweight=leaf_weight)
        else:
    ...
    nshapes = dict((('circle', '.'), ('square', 's'), ('sphere', 'o')))
    ...
    for shape in shapes:
        ...
        scat = axe.scatter([nodex[i] for i in indexes],
                           ...
                           s=ms, marker=nshapes.get(shape, shape))
